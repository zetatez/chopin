...
main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, ":oecmrhv", longopts, NULL)) != -1) {
        switch(opt) {
            case 'o': action = MagicOpen                                               ; break;
            case 'e': action = MagicExec                                               ; break;
            case 'c': action = MagicCopy                                               ; break;
            case 'm': action = MagicMove                                               ; break;
            case 'r': action = MagicRemove                                             ; break;
            case 'h': help()                                                           ; return EXIT_SUCCESS;
            case 'v': version()                                                        ; return EXIT_SUCCESS;
            default : fprintf(stderr, "%s: option '%c' is invalid\n", argv[0], optopt) ; return EXIT_FAILURE;
        }
    }
    ...
    switch (action) {
        case MagicOpen:
            flag = magicOpen(filename); break;
        case MagicExec:
            flag = magicExec(filename); break;
        case MagicCopy:
            flag = magicCopy(filename); break;
        case MagicMove:
            flag = magicMove(filename); break;
        case MagicRemove:
            flag = magicRemove(filename); break;
        default:
...
std::string
get_file_mine_type(const std::string& filename) {
...
int
magicOpen(const std::string filename) {
...
int
magicExec(const std::string filename) {
    if (filename.length() == 0) { return 1; }
...
int
magicCopy(const std::string filename) {
    if (filename.length() == 0) { return 1; }
...
int
magicMove(const std::string filename) {
    if (filename.length() == 0) { return 1; }
...
int
magicRemove(const std::string filename) {
    if (filename.length() == 0) { return 1; }
